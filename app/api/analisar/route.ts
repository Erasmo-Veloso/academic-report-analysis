...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!cohereResponse.ok) {
      ...
      if (cohereResponse.status === 429) {
        return NextResponse.json(
          {
            details:
              "Limite de requisições atingido. Aguarde alguns momentos e tente novamente.",
          },
          { status: 429 },
        );
      }
    ...
    console.log("[v0] Resposta Cohere:", JSON.stringify(cohereData, null, 2));
    ...
    if (!analysisText) {
      console.error("[v0] Estrutura da resposta Cohere:", cohereData);
      return NextResponse.json(
